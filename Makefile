...
attackstats: build-attack
	mkdir -p results/attackstats/last_lines
	docker build -f Dockerfile.attackstats -t hqc-attackstats .
	docker run -it --rm -v "$$(realpath results/attackstats):/collect_attack_stats/results" --entrypoint=../run.sh hqc-attackstats 10
