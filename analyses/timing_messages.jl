...
# ╔═╡ 41a3ade0-87cc-11eb-2da8-abea826c4d7b
using Plots, PGFPlotsX, DataFrames, DataFramesMeta, CSV, Statistics, DSP, StatsPlots, StatsBase

# ╔═╡ 633b2e92-868c-11eb-2552-9bb5eb2ff8d2
pgfplotsx()

# ╔═╡ 6527947a-868c-11eb-293e-d7dd2e0c9006
df = CSV.File("../timings/timings_messages_original.csv") |> DataFrame;

# ╔═╡ 1fd2d81c-8975-11eb-2541-2dfe7c4c1a85
median(df.time)

# ╔═╡ 9c707bf4-868c-11eb-38e7-a766fa8d396d
µ, σ = mean(df.time), std(df.time)

# ╔═╡ 5412d77a-868d-11eb-14c2-a98d43ad149b
...
medcycles = [median(group.time) for group in grouped_filtered_df]

# ╔═╡ 52d6c252-8829-11eb-34db-850a6fcfb790
modecycles = [mode(group.time) for group in grouped_filtered_df]

# ╔═╡ 7c7973e6-875e-11eb-3972-453745bcbff7
medcycles[2:end] .- medcycles[1:end-1]

# ╔═╡ 63fb4446-87c4-11eb-00e0-c9af991f54e3
lims = extrema(filtered_df.time)

# ╔═╡ 359335a2-873b-11eb-2a88-7112e9c476cb
begin
	p = density(
		filtered_df.time,
		xlab="cycles",
...
# ╔═╡ 808f957a-87ca-11eb-203b-b15d279e9663
possible_sexps = collect(map(keys(grouped_filtered_df)) do x
		x[1]
	end)

# ╔═╡ ee6b4748-87c4-11eb-2c92-c927b9728374
begin
	p2 = density(
		[group.time for group in grouped_filtered_df],
		labels=reshape(["$(j) iterations" for j in possible_sexps], 1, :),
		xlab="cycles",
		ylab="density",
		width=2,
		xlim=lims)
	#vline!(p2, modecycles, labels=false)
	# f = @where(filtered_df, :sexp.==3)
	# num_measurements = 10
	# r = rand(1:length(f.time)-num_measurements)
	# vline!(p2, [median(f.time[r:r+num_measurements])], color=:red, width=3)
	savefig(p2, "cycles_conditional_probability.pdf")
	p2
end

# ╔═╡ eb76958c-87cf-11eb-1327-cbd76db80443
density(filtered_df.time, width=2, xlab="cycles", ylab="density", xlim=lims)
